...
const $$SelectForm = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div${addAttribute(`formGroup relative mb-10 d-flex ${ClassName}`, "class")}> ${title && renderTemplate`<label${addAttribute(id, "for")} class="pr-4 pb-5"> ${title}</label>`} <select${addAttribute(id, "name")}${addAttribute(id, "id")} class="round-8 pr-5 pl-5"> ${renderSlot($$result, $$slots["default"])} </select> </div>`;
